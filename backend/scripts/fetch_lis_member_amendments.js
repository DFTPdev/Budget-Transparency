...
const cfgPath = path.resolve(baseDir, 'lis.config.json');
const cfg = JSON.parse(fs.readFileSync(cfgPath, 'utf8'));

const INDEX_URL = cfg.memberIndexUrl;
const RAW_DIR = path.resolve(baseDir, cfg.downloadDir || './lis_downloads');
...
function parseHtmlToRecords(html, fallbackSponsor) {
  ...
  $('table').each((_, table) => {
    ...
    $table.find('tbody tr, tr').each((_, tr) => {
      ...
      if (Number.isFinite(amount)) out.push({ sponsor, amount, desc });
    });
  });
  if (out.length === 0) {
    $('li, p').each((_, el) => {
